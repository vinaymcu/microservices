...
public class DeactivateMessageTest {
	@InjectMocks
	...
	public void testDeactivateMessage()
	{
		...
		when(messageValidator.validateDeactivateMessage(null, message.getMessageId())).thenReturn(getResultObject());
		GenericResponse response= unicastServiceImpl.deactivateMessage(messageIds);
		assertEquals("ACN_9204", response.getResultObject().get(0).getResultCode());
	}
	
	@Test // message id that does not exist in db
	public void testDeactivateMessage2()
	{
